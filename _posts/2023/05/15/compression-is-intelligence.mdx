---
title: "压缩即智慧？"
tags: ["machinelearning", "insight", "ai", "gpt", "tech"]
description: "本文探讨了大型语言模型（LLM）如 ChatGPT 本质上是文本的有损压缩和解压算法的观点。文章分析了 GPT 的压缩本质、插值算法与幻觉问题的关系，以及人类智慧本身就是信息压缩能力的体现。文章还引用了 OpenAI 首席科学家对 '通过压缩实现无监督学习' 的观点，探讨了这一理论对 AI 发展的含义。"
...
- 互联网上的文本内容，可以认为是高维空间的人类知识在一维空间上的“投影”，GPT-like 的压缩算法是根据这些投影出来的信息，建立起的一个高维空间（XX 亿参数）的模型。
...
- 可以把所有 ChatGPT 的回答都当做它在用自己的有损信息做“插值算法”，产生最能符合你要求的结果，当你要求 ChatGPT”使用独立宣言的风格描述在烘干机中丢失袜子”时，他其实是在“词汇空间”中取两个点并生成占据它们之间位置的文本：_When in the Course of human events, it becomes necessary for one to separate his garments from their mates, in order to maintain the cleanliness and order thereof ……_ 正是类似图像补全算法中的“插值算法” —— 这种把有损信息补全的方式让我们产生了 GPT/LLM拥有“智慧”的错觉。
